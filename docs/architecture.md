...
模式策略按 `mode + phase` 组织，阶段包括 `question`、`reveal`、`answerReview` 和 `summary`。模拟面试另外包含一份完整业务上下文，覆盖简历与 JD 对齐、项目验证、追问路径、手撕代码、回答判定、中途操作和总结边界；它只在会话首次激活该模式时注入一次，不放进工具描述，也不在每次操作中重复注入。看答案与作答后点评使用不同策略：前者不创建作答、不生成评分，后者基于真实回答生成评分、点评和讲解。自然语言路径复用当前会话已经激活的模式上下文；通过原子 create 或 bind 首次激活练习时，业务结果会携带一次模式上下文，后续原子操作不重复注入。
