...
const LEETCODE_MATERIAL_RULES = [
  '【题目材料】每道力扣题第一次展示前，必须先调用 interview_materials create 保存题目材料：题意、示例、数据范围、前置知识、分级提示、常见误区、相似题。',
  '【材料展示】材料必须通过 interview_show_question 返回的 materialsFence 原样展示为 dsh-ui 围栏，写在回复正文里；禁止改写、删减或重新排版围栏内容，禁止改用普通文本复述材料。',
  '【提示阶梯】提示按由浅入深保存为 3 到 4 级。用户没有要求时不要主动给出任何一级提示；用户点题目卡的「提示」按钮或明确要提示时，先读取当前题已解锁到第几级，再顺着下一级讲下去，不要一次给完所有提示。',
  '【知识点】用户基础薄弱时优先补前置知识：数据结构、算法模板、复杂度量级和常见误区都要用可以直接听懂的话讲清楚。',
].join('')
